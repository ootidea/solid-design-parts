...
export function Calendar(rawProps: CalendarProps) {
  ...
  const firstDateOfSelectedCalendar = () =>
    firstDateOfSelectedMonth().subtract(firstDateOfSelectedMonth().day(), "day");
      ...
      <div class="skel-Calendar_year-month-area">
        ...
        <div class="skel-Calendar_year-month">
          <span class="skel-Calendar_year">
            {selectedMonth_().format(i18n.literals.calendarYearTemplate)}
          </span>
          <span class="skel-Calendar_month">
            {selectedMonth_().format(i18n.literals.calendarMonthTemplate)}
          </span>
        </div>
        <IconButton
          ...
          {(weakIndex) => (
            <div class="skel-Calendar_date-row">
              <For each={dayNames}>
                {(_, day) => {
                  const date = () =>
                    firstDateOfSelectedCalendar().add(weakIndex, "week").add(day(), "day");
                  return (
                    <div
                      class="skel-Calendar_cell"
                      classList={{
                        "skel-Calendar_other-month":
                          date().isAfter(selectedMonth(), "month") ||
                          date().isBefore(selectedMonth(), "month"),
                      }}
                      data-day={day()}
                    >
